...
public class DownloadManagerPro {
    ...
    public void getDownloadList(DownloadListCallback callback, String accessAll) {
        DownloadManager.Query query = new DownloadManager.Query();
        Cursor cursor = null;
        if(!accessAll.isEmpty()){
            if(accessAll.equals("1")) {
            	ReflectUtil.invorkMethod(DownloadManager.class, downloadManager, "setAccessAllDownloads", true);
//                downloadManager.setAccessAllDownloads(true);
        ...
        try {
            ...
            if (cursor != null && (cursor.getCount() > 0) && cursor.moveToFirst()) {
               do{
                   DownloadFile file = getDownloadFile(cursor);
                   Gson gson = new Gson();
                   String msg = gson.toJson(file);
                   callback.onDownloadFile(msg);
               }while(cursor.moveToNext());
            }else {
                callback.onDownloadFile("");
            }
        } catch (Exception e) {
            e.printStackTrace();
        }finally {
    ...
    public String getDownloadFileInfo(DownloadFileCallback callback, long downloadId) {
        String fileInfo = "";
        DownloadManager.Query query = new DownloadManager.Query().setFilterById(downloadId);
        Cursor cursor = null;
        try {
            ...
            if(callback != null) {
                callback.onDownloadFileInfo(fileInfo);
            }
        }
        return fileInfo;
    }

    private DownloadFile getDownloadFile(Cursor cursor) {
        ...
        if(cursor != null) {
            ...
            long lastmod = cursor.getLong(cursor.getColumnIndex(DownloadManager.COLUMN_LAST_MODIFIED_TIMESTAMP));
            long current = cursor.getLong(cursor.getColumnIndex(DownloadManager.COLUMN_BYTES_DOWNLOADED_SO_FAR));
            ...
            downloadFile = new DownloadFile(id, url, type, name, lastmod, status, total, current,path);
        }
        return downloadFile;
    }
}
